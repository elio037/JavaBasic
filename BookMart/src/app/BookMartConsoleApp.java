...
public class BookMartConsoleApp {
	...
	private void controlCartMenu() {
		...
		do {
			...
			//"돌아가기", "도서 주문", "도서 삭제", "장바구니 비우기"}
			switch (menu) {
			case 1 : menuCartOrder(); break;
			case 2 : menuCartBookDelete(); break;
	...
	private void menuCartOrder() {
		...
		for (CartItemVO item : cs.listCartItems()) {
			...
			totalPrice += price;
			orderItemList.add(
				new OrderItemVO(item.getBookNo(), item.getQuantity(), price)
			);
		}
	...
	private void menuCartBookDelete() {
		...
		if (item == null) {
			...
			System.out.println("장바구니에서 도서를 삭제하였습니다.");
		}
		displayCartItemList();
	}
	...
	private void menuCartClear() {
		...
		String confirm = input.readString(">> 장바구니의 모든 도서를 삭제하시겠습니까? ('" + CONFIRM + "'이면 삭제) : ");
		if (confirm.equals(CONFIRM)) {
			cs.clearCart();
